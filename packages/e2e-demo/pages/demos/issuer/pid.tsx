...
export const getServerSideProps = requireAuth<Props>(async (context) => {
  ...
  const qrCodeData = challengeTokenUrlWrapper(
    fullURL(`/api/demos/issuer/manifests/pid/${authToken}`)
  )
...
const IdentityPage: NextPage<Props> = ({
  createdAt,
  manifest,
  qrCodeData,
  user
  ...
  const content =
    data && data.credential ? (
      <div className="space-y-4">
        ...
        <h3>Decoded PID Verifiable Credential:</h3>
        <pre>{JSON.stringify(data.credential, null, 4)}</pre>
      </div>
    ) : (
      <>
        <h2>PID Verifiable Credential Issue User Experience</h2>
        <p className="pb-4">
        ...
        />
        <pre> {qrCodeData["challengeTokenUrl"]}</pre>
